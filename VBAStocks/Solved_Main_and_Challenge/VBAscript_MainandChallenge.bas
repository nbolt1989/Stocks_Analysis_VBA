...
'Headers and Row Titles for Challenge-------
...
'Now instead of cycling through the original table, I will need to cycle through my summary table to find greatest increase and decrease and total volume
'I will need to define a new last row but for my new table.
    new_sumtable_last = WS.Cells(Rows.Count, 9).End(xlUp).Row

'I need a forloop like my above forloop through the tickers of my sum table but I will not need (<>) since we have all tickers separated
        For c = 2 To new_sumtable_last
            If Cells(c, 11).Value = Application.WorksheetFunction.Max(WS.Range("K2:K" & new_sumtable_last)) Then
                Range("O2").Value = Cells(c, 9).Value
                Range("P2").Value = Cells(c, 11).Value
                Range("P2").NumberFormat = "0.00%"
            ElseIf Cells(c, 11).Value = Application.WorksheetFunction.Min(WS.Range("K2:K" & new_sumtable_last)) Then
                Range("O3").Value = Cells(c, 9).Value
                Range("P3").Value = Cells(c, 11).Value
                Range("P3").NumberFormat = "0.00%"
            ElseIf Cells(c, 12).Value = Application.WorksheetFunction.Max(WS.Range("L2:L" & new_sumtable_last)) Then
                Range("O4").Value = Cells(c, 9).Value
                Range("P4").Value = Cells(c, 12).Value
                End If
